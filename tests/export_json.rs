...
"#,
    ...
    let joined_path = join_paths(once(fake_bin).chain(split_paths(&host_path)))
        .expect("test PATH should be valid");
    let path_string = joined_path.to_string_lossy().to_string();
    let profile_log_string = profile_log.to_string_lossy().to_string();
    ...
    let out = sb.run(
        ...
        &[
            ("CADE_DIRENV", "full"),
            ("PATH", &path_string),
            ("CADE_FAKE_NIX_PROFILE_LOG", &profile_log_string),
        ],
    );
    assert!(out.status.success(), "{out:?}");
    assert!(stderr(&out).is_empty(), "{}", stderr(&out));
    ...
    assert!(
        json["PATH"]
            .as_str()
            .is_some_and(|candidate| candidate.starts_with("/materialized/bin:")),
        "{json}"
    );

    let logged_profile = read_to_string(&profile_log_string).unwrap();
    assert!(
